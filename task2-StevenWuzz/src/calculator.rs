...
impl Calculator {
    fn calculate(&self, x: i64, y: i64, operation_type: &OperationType) -> Option<i64> {
        operation_type.perform(x, y)
    }
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        self.calculate(x, y, &OperationType::Addition).map(|result| {
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        self.calculate(x, y, &OperationType::Subtraction).map(|result| {
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        self.calculate(x, y, &OperationType::Multiplication).map(|result| {
    ...
    pub fn show_history(&self) -> String{
        ...
        for (index, operation) in self.history.iter().enumerate() {
            history_str.push_str(&format!("{}: {} {} {} = {}\n", 
                ...
                self.calculate(operation.first_num, operation.second_num, &operation.operation_type).unwrap()));
        }
        history_str
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        ...
        let result = self.calculate(operation.first_num, operation.second_num, &operation.operation_type).unwrap();
